...
- Track packwiz descriptors such as `mods/*.pw.toml`. Root `pack.toml`, `index.toml`, and `.packwizignore` are generated by `devtool.bat prepare-pack` / `refresh`; keep the tracked templates in `pack/`.
