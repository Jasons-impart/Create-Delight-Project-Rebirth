...
- Track packwiz source metadata: `*.pw.toml` and the release-root templates in `pack/`, including `pack/.packwizignore.source`. Root `pack.toml`, `index.toml`, and `.packwizignore` are generated local files.
...
6. Review `pack/pack.toml`, `pack/.packwizignore.source`, and any generated `*.pw.toml` before finalizing. Root `pack.toml`, `index.toml`, and `.packwizignore` are generated by devtool/packwiz and are not committed.
...
Root `index.toml` and `pack.toml` are generated local files; mention changes to tracked packwiz source files instead.
